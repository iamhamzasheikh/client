...
const Header = () => {
  ...
  const onClickHandler = () => {
    // Show the login modal or perform any other action
  ...
  return (
    ...
    >
      ...
      mx-auto mt-10 text-center'>Turn text to <motion.span className='text-blue-600'
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      <motion.div
        ...
        {Array(6).fill('').map((item, index) => (
          <motion.img
            whileHover={{ scale: 1.05, duration: 0.1 }} className='rounded hover:scale-105 transition-all duration-300 cursor-pointer
          max-sm:w-10' src={index % 2 === 0 ? assets.sample_img_2 : assets.sample_img_1} key={index} width={70} />
        ))}
      </motion.div>
